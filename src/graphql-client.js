...
function getPlantId(plantId) {
  if (typeof window !== 'undefined') {
    // get the plant id from local storage if it exists
    const plant = window.localStorage.getItem(plantId);
    // return the headers to the context so httpLink can read them
    return plant;
  }
  return '';
...
export function createApolloClient({
  ...
  if (!disableHttp) {
    ...
    authLink = setContext((_, { headers }) => {
      ...
      const plant = getPlantId(plantId);
      let authorizationHeader = authorization ? { authorization } : {};
      authorizationHeader = plant ? { plant, ...authorizationHeader } : authorizationHeader;
      return {
        headers: {
          ...headers,
          ...authorizationHeader,
    ...
    if (wsEndpoint) {
      wsClient = new SubscriptionClient(wsEndpoint, {
        ...
        connectionParams: () => {
          const authorization = getAuth(tokenName);
          const plant = getPlantId(plantId);
          let param = authorization ? { authorization } : {};
          param = plant ? { plant, ...param } : param;
          return param;
        },
      });
